...
# default command
cmd="singularity run -e -B /om2 -B ${scratch}:/workdir -B ${bids_dir}:/input:ro -B ${outdir}:/out ${SING_IMG} /input /out participant --participant_label ${subject} -w /workdir --cifti-output 91k --mem_mb 40000 --output-spaces MNI152NLin6Asym:res-2 anat --fs-subjects-dir /out/freesurfer-${session}/ --bids-filter-file bids-filter-${session}.json --bids-database-dir /out/bids_db/ --nprocs 16 --omp-nthreads 8 --reports-only"
